...
local function getTextureAndRange(szImage)
	...
	local sprite = cc.Sprite:create(szImage)
...
--	FrameCache:addSpriteFrames(pList)
	...
	local function touchMove(touch, event)
		...
		layer.ratioVal = cc.clampf(layer.ratioVal + dy/100, 0.0, 0.9) --最大程度默认0.9
		posNow = location
		return true
	end
	...
	local function draw(transform, transformUpdated)
		...
		for _, v in ipairs(cardMesh) do 
			...
			local progress = 0
			if layer.ratioVal > 0.8 then
				progress = 1
			end
			print("Pregress"..progress)
			local width = gl.getUniformLocation(glProgram:getProgram(), "finish")
			glProgram:setUniformLocationF32(finish,progress)
